...
public class Node {
    ...
    private static HashMap<Coordinate, Box> boxesByCoordinate1 = new HashMap<Coordinate, Box>();
	...
	public static HashMap<Box, HashMap<Coordinate, Integer>> boxDistance = 
			new HashMap<Box, HashMap<Coordinate, Integer>>();
	...
	public static void computeBoxDistance() {
		for (Box box : Node.getBoxesByCoordinate1().values()) {
			HashMap<Coordinate, Integer> distanceMap = new HashMap<Coordinate, Integer>();
			LinkedList<Coordinate> frontier = new LinkedList<Coordinate>();
			HashSet<Coordinate> frontierHash = new HashSet<Coordinate>();
			// initially put only the goal in the distance map
			distanceMap.put(box.getCoordinate(), 0);
			// and the goal's neighbours (4-vicinity) in the frontier
			for (Coordinate coordinate : box.getCoordinate().get4VicinityCoordinates()) {
				if (Node.walls.get(coordinate) == null && 
						coordinate.getRow() > -1 && coordinate.getRow() < Node.totalRows &&
						coordinate.getColumn() > -1 && coordinate.getColumn() < Node.totalColumns) {
					frontier.add(coordinate);
			...
			Node.boxDistance.put(box, distanceMap);
		}
	...
	public static void showDistance(){
		 ...
		 */
			...
			Iterator<HashMap.Entry<Goal,HashMap<Coordinate, Integer>>> iterGoals = goalDistance.entrySet().iterator();
			while (iterGoals.hasNext()) {
				Entry<Goal, HashMap<Coordinate, Integer>> entry = iterGoals.next();
				Goal goal = (Goal) entry.getKey();
				HashMap<Coordinate, Integer> coorDis = entry.getValue();
				System.err.println("Goal " + goal.getLetter() + ": ");
				Iterator<HashMap.Entry<Coordinate, Integer>> iterDistance = coorDis.entrySet().iterator();
				while(iterDistance.hasNext()){
					Entry<Coordinate, Integer> entryDistance = iterDistance.next();
					Coordinate coor = (Coordinate) entryDistance.getKey();
					Integer distance = entryDistance.getValue();
					System.err.print("( "+coor.getColumn()+","+ coor.getRow()+") -> " + distance + ";   ");
				}
				System.err.println();
			}
			
			System.err.println();
			System.err.println("------The Boxes Distance MARIX------");
			Iterator<HashMap.Entry<Box,HashMap<Coordinate, Integer>>> iterBoxes = boxDistance.entrySet().iterator();
			while (iterBoxes.hasNext()) {
				Entry<Box, HashMap<Coordinate, Integer>> entry = iterBoxes.next();
				Box box = (Box) entry.getKey();
				HashMap<Coordinate, Integer> coorDis = entry.getValue();
				System.err.println("Box " + box.getLetter() + ": ");
				Iterator<HashMap.Entry<Coordinate, Integer>> iterDistance = coorDis.entrySet().iterator();
				while(iterDistance.hasNext()){
					Entry<Coordinate, Integer> entryDistance = iterDistance.next();
					Coordinate coor = (Coordinate) entryDistance.getKey();
	...
	public static HashMap<Coordinate, Box> getBoxesByCoordinate1() {
		return boxesByCoordinate1;
	}

	public static HashMap<Character, Box> getBoxesByID1() {
		return boxesByID1;
	}
	
	public static void addBox1(Box box) {
        Node.boxesByCoordinate1.put(box.getCoordinate(), box);
        Node.boxesByID1.put(box.getLetter(), box);
    }
	...
	public static void setGoalsPriority(){
		...
		while (iterGoals.hasNext()) {
			...
			while(iterDistance.hasNext()){
				...
				Coordinate coor = (Coordinate) entryDistance.getKey();
				Integer distance = entryDistance.getValue();
				if(agent0.getCoordinate().getRow() == coor.getRow() 
						&& agent0.getCoordinate().getColumn() == coor.getColumn()){
					priority.put(goal, distance);
					distanceList.add(distance);
					System.err.println("Distance from goal to Agent 0 : Goal -> " + goal.getLetter() + " , distance -> " + distance);
				}
			}
		}
			...
			if(i==0 || distanceList.get(i) != distanceList.get(i-1) ){
				...
				while(iterPriority.hasNext()){
					...
					if(enrtyPriority.getValue() == distanceList.get(i)){
						...
						tempCount ++ ;
						System.err.println("Goal " + enrtyPriority.getKey().getLetter() + "\'s priority " + enrtyPriority.getKey().getPriority());
					}
				}
			}
		}
			...
			if(i==0 || distanceList.get(i) != distanceList.get(i-1) ){
				...
				while(iterPriority.hasNext()){
					...
					if(enrtyPriority.getValue() == distanceList.get(i)){
						...
						System.err.println("Goal " + goal.getLetter() + "\'s intially priority is : " + goal.getPriority());
						HashMap<Coordinate, Boolean> wallsTemp = (HashMap<Coordinate, Boolean>) Node.walls.clone();
						wallsTemp.put(goal.getCoordinate(), true);
						System.err.println("Now the goal " + goal.getLetter() + " is assumed as wall !");
						/*
						 * Calculating all distances from agent0 to other goals
						 */
						HashMap<Coordinate, Integer> agent0DistanceMap = new HashMap<Coordinate, Integer>();
						LinkedList<Coordinate> frontier = new LinkedList<Coordinate>();
						HashSet<Coordinate> frontierHash = new HashSet<Coordinate>();
						// initially put only the goal in the distance map
						agent0DistanceMap.put(agent0.getCoordinate(), 0);
						// and the goal's neighbours (4-vicinity) in the frontier
						for (Coordinate coordinate : agent0.getCoordinate().get4VicinityCoordinates()) {
						...
						while (!frontier.isEmpty()) {
							...
							Integer minDistance = Integer.MAX_VALUE;
							for (Coordinate neighbour : coordinate.get4VicinityCoordinates()) {
								if (agent0DistanceMap.containsKey(neighbour) && agent0DistanceMap.get(neighbour) < minDistance) {
									minDistance = agent0DistanceMap.get(neighbour);
								}
							}
							agent0DistanceMap.put(coordinate, minDistance+1);
							for (Coordinate neighbour : coordinate.get4VicinityCoordinates()) {
								if (wallsTemp.get(neighbour) == null && 
										neighbour.getRow() > -1 && neighbour.getRow() < Node.totalRows &&
										neighbour.getColumn() > -1 && neighbour.getColumn() < Node.totalColumns &&
										!agent0DistanceMap.containsKey(neighbour) &&
										!frontierHash.contains(neighbour)) {
									frontier.add(neighbour);
									frontierHash.add(neighbour);
								}
							...
							if(j==0 || distanceList.get(j) != distanceList.get(j-1) ){
								...
								while(iterPriority1.hasNext()){
									...
									if(enrtyPriority1.getValue() == distanceList.get(j)){
										...
										if(enrtyPriority1.getKey().getLetter() == goal.getLetter()){
											isGoalExist = true ;
										} else{
											Iterator<HashMap.Entry<Coordinate, Integer>> distance = agent0DistanceMap.entrySet().iterator();
											while (distance.hasNext()) {
												Entry<Coordinate, Integer> entry3 = distance.next();
												if(entry3.getKey().getColumn() == enrtyPriority1.getKey().getCoordinate().getColumn() 
														&& entry3.getKey().getRow() == enrtyPriority1.getKey().getCoordinate().getRow()){
													isGoalExist = true;
												}
											}
										}
										...
										if(!isGoalExist){
											System.err.println("ATTENTION : The goal " + enrtyPriority1.getKey().getLetter() + " has been blocked!");
				...
				//							}
						...
						System.err.println("Goal " + goal.getLetter() + "\'s finally priority is : " + goal.getPriority());
		...
		for(int i=0; i<distanceList.size(); i++){
			if(i==0 || distanceList.get(i) != distanceList.get(i-1) ){
				...
				while(iterPriority.hasNext()){
					Entry<Goal, Integer> enrtyPriority = iterPriority.next();
					if(enrtyPriority.getValue() == distanceList.get(i)){
						System.err.println("Goal " + enrtyPriority.getKey().getLetter() + "\'s final priority " + enrtyPriority.getKey().getPriority());
					}
				}
			}
		}
